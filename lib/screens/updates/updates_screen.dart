...
class _UpdatesScreenState extends ConsumerState<UpdatesScreen>
  ...
  int get _newStockLevel {
    ...
    if (_transactionType == 'STOCK IN') {
      return _selectedProduct!.quantityAvailable + _quantity;
    } else {
      return (_selectedProduct!.quantityAvailable - _quantity).clamp(0, 999999);
    }
  }

  double get _newStockPercent {
    if (_selectedProduct == null) return 0;
    final maxCapacity = _selectedProduct!.minimumThreshold * 3;
    if (maxCapacity <= 0) return _newStockLevel > 0 ? 1.0 : 0.0;
    return (_newStockLevel / maxCapacity).clamp(0.0, 1.0);
  }
  ...
  Future<void> _confirmTransaction() async {
    ...
    if (_transactionType == 'STOCK OUT' &&
        _quantity > _selectedProduct!.quantityAvailable) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
              'Cannot remove $_quantity units. Only ${_selectedProduct!.quantityAvailable} available.'),
    ...
    final prev = _selectedProduct!.quantityAvailable;
    ...
    await ref.read(transactionProvider.notifier).addTransaction(
          ...
          transactionType: _transactionType == 'STOCK IN' ? 'stock_in' : 'stock_out',
          quantity: _quantity,
          updatedBy: _selectedUser,
  ...
  Widget _buildLivePreview() {
    final current = _selectedProduct!.quantityAvailable;
    final next = _newStockLevel;
